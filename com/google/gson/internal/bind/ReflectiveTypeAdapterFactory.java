...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory {
   ...
   private ReflectiveTypeAdapterFactory.BoundField createBoundField(Gson context, Field field, Method accessor, String serializedName, TypeToken<?> fieldType, boolean serialize, boolean blockInaccessible) {
      ...
      final Object writeTypeAdapter;
      if (serialize) {
         writeTypeAdapter = jsonAdapterPresent ? mapped : new TypeAdapterRuntimeTypeWrapper(context, mapped, fieldType.getType());
      } else {
         ...
         for(Class originalRaw = raw; raw != Object.class; raw = type.getRawType()) {
            ...
            for(int var12 = 0; var12 < var11; ++var12) {
               ...
               if (serialize || deserialize) {
                  ...
                  ReflectiveTypeAdapterFactory.BoundField boundField = this.createBoundField(context, field, accessor, serializedName, TypeToken.get(fieldType), serialize, blockInaccessible);
                  if (deserialize) {
                     Iterator var21 = fieldNames.iterator();

                     while(var21.hasNext()) {
                        String name = (String)var21.next();
                        ReflectiveTypeAdapterFactory.BoundField replaced = (ReflectiveTypeAdapterFactory.BoundField)deserializedFields.put(name, boundField);
                        if (replaced != null) {
                  ...
                  if (serialize) {
                     ReflectiveTypeAdapterFactory.BoundField replaced = (ReflectiveTypeAdapterFactory.BoundField)serializedFields.put(serializedName, boundField);
                     if (replaced != null) {
   ...
   public abstract static class Adapter<T, A> extends TypeAdapter<T> {
      ...
      public void write(JsonWriter out, T value) throws IOException {
         if (value == null) {
            ...
            try {
               Iterator var3 = this.fieldsData.serializedFields.iterator();
      ...
      public T read(JsonReader in) throws IOException {
         if (in.peek() == JsonToken.NULL) {
            ...
            Map deserializedFields = this.fieldsData.deserializedFields;
            ...
            try {
               ...
               while(in.hasNext()) {
                  ...
                  ReflectiveTypeAdapterFactory.BoundField field = (ReflectiveTypeAdapterFactory.BoundField)deserializedFields.get(name);
                  if (field == null) {
                     in.skipValue();
                  } else {
   ...
   private static class FieldsData {
      public static final ReflectiveTypeAdapterFactory.FieldsData EMPTY = new ReflectiveTypeAdapterFactory.FieldsData(Collections.emptyMap(), Collections.emptyList());
